...
__global__ void MatMul_SharedMem(DATA_TYPE* matA, DATA_TYPE* matB, DATA_TYPE* matC, int m, int n, int k)
{
	...
	int globalRow = blockDim.x * blockIdx.x + threadIdx.x;
	...
	for (int bId = 0; bId < ceil(float(k) / BLOCK_SIZE); bId++){
		...
		// load subA and subB
		if (globalRow >= m || stride + localCol >= k)
			subA[localRow][localCol] = 0;
		else
			subA[localRow][localCol] = matA[k * globalRow + (stride + localCol)];
		
		if (globalCol >= n || stride + localRow >= k)
			subB[localRow][localCol] = 0;
		else
			subB[localRow][localCol] = matB[n * (stride + localRow) + globalCol];
		...
		// to global memory
		matC[globalRow * n + globalCol] = val;
